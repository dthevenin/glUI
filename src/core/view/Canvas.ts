...
export class Canvas extends View {
   ...
   */
  __canvas_ctx: CanvasRenderingContext2D = null;
   ...
   */
  __canvas_node: HTMLCanvasElement = null;
 ...
 ****************************************************************/
   ...
   */
  drawRect(x: number, y: number, w: number, h: number, radius: number, rotation: number): Canvas {
    if (arguments.length === 4) {
      ...
      if (!rotation) {
        ...
        const points = [x,y];
        let p = this.rotatePoint(w, 0, rotation);
  ...
  c_ellipse(
    ...
    rotation: number = 0, sa: number = 0, ea: number = 2 * Math.PI,
   ...
   */
  draw(x: number = 0, y: number = 0, width?: number, height?: number) {
...
const setup = () => {
  ...
  CANVAS_METHODS.forEach(m => {
    // @ts-ignore
    Canvas.prototype["c_" + m] = ((m: string) => function (...args: any[]): Canvas { // 9 args is most in API
      this.__canvas_ctx[m].apply(this.__canvas_ctx, ...args);
      return this;
    })(m);
  });

  CANVAS_PROS.forEach(p => {
    const d: PropertyDescriptor = {
      enumerable: true,
      set: ((p) => function (value: any) { this.__canvas_ctx[p] = value;})(p),
      get: ((p) => function () { return this.__canvas_ctx[p];})(p),
    };

    Object.defineProperty(Canvas.prototype, "c_" + p, d);
  });
};
